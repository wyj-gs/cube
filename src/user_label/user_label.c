...
{
	int ret;
	RECORD(MAC_LABEL,ADDR) * addr_label;
	...
	db_record=memdb_find(conn_uuid,TYPE_PAIR(MAC_LABEL,ADDR));
	//db_record=memdb_find_first(TYPE_PAIR(MAC_LABEL,ADDR),"proc_uuid",conn_uuid);
	if(db_record==NULL)
	{
	...
	//	Memcpy(addr_label->proc_uuid,conn_uuid,DIGEST_SIZE);
		...
		memdb_store(addr_label,TYPE_PAIR(MAC_LABEL,ADDR),NULL);
	}
	else
	{
		addr_label=(RECORD(MAC_LABEL,ADDR) *)db_record->record;
...
int proc_login_addrattach(void * sub_proc,void * recv_msg)
{
	...
	db_record=memdb_find(proc_uuid,TYPE_PAIR(MAC_LABEL,ADDR));
	//db_record=memdb_find_first(TYPE_PAIR(MAC_LABEL,ADDR),"proc_uuid",proc_uuid);
	if(db_record==NULL)
	{
	...
	ret=message_add_expand_data(recv_msg,TYPE_PAIR(MAC_LABEL,ADDR),addr_label);
	if(ret<0)
		return ret;
...
int proc_login_addrverify(void * sub_proc,void * recv_msg)
{
	...
	ret=message_remove_expand(recv_msg,TYPE_PAIR(MAC_LABEL,ADDR),&msg_expand);
	if(ret<0)
		return ret;
	if(msg_expand==NULL)
	{
...
int proc_login_userattach(void * sub_proc,void * recv_msg)
{
	...
	db_record=memdb_find_first(TYPE_PAIR(MAC_LABEL,SUBJECT),"user_name",addr_label->user_name);
	if(db_record==NULL)
	{
    ...
    char text[256];
    struct_2_json(user_label,text,memdb_get_template(TYPE_PAIR(MAC_LABEL,SUBJECT)));
